			...
			<form class="form-inline" action="leaveListPaging.action" method="post" id="leaveListPaging">
				<div class="form-group">
					<label for="exampleInputEmail2">&nbsp;&nbsp; 开始时间：</label>
					 <input type="date" class="form-control input-sm" name="leave.begindate" value="<fmt:formatDate value="${leave.begindate}"/>">
				</div>
				<div class="form-group">
					<label for="exampleInputName3">&nbsp;&nbsp;结束时间：</label> <input
						type="date" class="form-control input-sm" id="exampleInputName3"
			...
			<table class="table table-bordered table-hover text-center">
				...
				<c:forEach var="leave" items="${paging.list}">
					<tr>
						...
						<td><fmt:formatDate value="${leave.begindate}"/></td>
						<td>
							<c:if test="${leave.leaveResult.shdate==''||leave.leaveResult.shdate==null}">无</c:if>
							<fmt:formatDate value="${leave.leaveResult.shdate}"/>
